  ...
  ngOnInit() {
	  this.brandingService.getBranding().subscribe(result => {
		...
		if (this.brandingConfigurations.length > 1) {
			...
			if (marketConfig)
				this.brandingSections = this.brandingConfigurations[this.brandingConfigurations.indexOf(marketConfig)].brandingElements;
	    } 
		...
		if (this.brandSectionNames.length > 0) {
			...
			for (let i = 0; i < this.brandSectionNames.length; i++) {
				menu.push(new NavMenuOption(this.brandSectionNames[i], null,
					{
						onClick: this.cs,
